...
produced by `basepair.cli.modisco.modisco_score2`
...
from basepair.cli.modisco import get_nonredundant_example_idx
import numpy as np
...
def profile_features(seqlets, ref_seqlets, profile, profile_width=70, profile_ref=None):
...
def annotate_profile(dfi, mr, profiles, profile_width=70, trim_frac=0.08, pattern_map=None, profiles_mr=None):
    ...
    for pattern in tqdm(dfi.pattern.unique()):
        ...
        for task in profiles:
            ...
            profile_ref = None if profiles_mr is None else profiles_mr[pattern.split("/")[0]][task]
