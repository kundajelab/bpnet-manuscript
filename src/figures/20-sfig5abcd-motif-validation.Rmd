---
title: "20-sfig5abcd-motif-validation: BPnet Cofactor Validation"
...
options(knitr.figure_dir="20-sfig5abcd-motif-validation", width=16, height=20)
...
## Esrrb ChIP at Esrrb motif (Non-BPnet)

### Find Esrrb motif locations across genome through sequence match

```{r motif locations esrrb, eval=F}
esrrb_motif_across_genome <- DNAString("TCAAGGTCA") %>% 
  vmatchPattern(., BSgenome.Mmusculus.UCSC.mm10, max.mismatch =0, fixed = "subject") 
saveRDS("data/rdata/motif_locations/esrrb_TCAAGGTCA_motif_instances_0_mm.rds")
```

### Plot matches

```{r sfig5b-esrrb-classic}
#Esrrb ChIP at Esrrb-specific motif
esrrb_motif_instances<-readRDS("data/rdata/motif_locations/esrrb_TCAAGGTCA_motif_instances_0_mm.rds")

#Switch strand to match BPnet orientation
neg_idx<-which(strand(esrrb_motif_instances)=="-")
pos_idx<-which(strand(esrrb_motif_instances)=="+")
strand(esrrb_motif_instances[neg_idx])<-"+"
strand(esrrb_motif_instances[pos_idx])<-"-"

#Select top 1K instances that score for Esrrb
esrrb_motif_instances$esrrb_score<-regionMeans(esrrb_motif_instances, bw.list$esrrb$pos) + 
  abs(regionMeans(resize(esrrb_motif_instances, 100, "center"), bw.list$esrrb$neg))
esrrb_motif_instances<-esrrb_motif_instances[order(esrrb_motif_instances$esrrb_score, decreasing=T)][1:1000] 

esrrb_chip_across_esrrb_motif<-plot_chipnexus_across_motifs(motif=resize(esrrb_motif_instances, 1), 
                                                            title = "Esrrb ChIP at Esrrb Motif \n(Non BPNet)", upstream = 500, downstream = 500, ylim=c(-.015, .015),
                                                            sample = bw.list$esrrb)
print(esrrb_chip_across_esrrb_motif)
```
...
```{r sfig5b-esrrb-classic-motif-logo}
esrrb_motif_instances_subset<-esrrb_motif_instances
esrrb_motif_instances_subset$seq<-getSeq(BSgenome.Mmusculus.UCSC.mm10, esrrb_motif_instances_subset)
esrrb_motif_instances_pwm<-PWM(esrrb_motif_instances_subset$seq, type="prob")
motif <- new("pcm", mat=as.matrix(esrrb_motif_instances_pwm), name="bin_SOLEXA")
plot(motif)
```
...
### Find Pbx motif locations across genome through sequence match

```{r motif locations, eval=F}
pbx_motif_across_genome <- DNAString("TGAKTGACAGG") %>% 
  vmatchPattern(., BSgenome.Mmusculus.UCSC.mm10, max.mismatch =0, fixed = "subject") 
saveRDS("data/rdata/motif_locations/pbx_TGAKTGACAGG_motif_instances_0_mm.rds")
```

## Compute top scoring Pbx motif locations 

```{r import pbx motif}
pbx_motif_instances<-readRDS("data/rdata/motif_locations/pbx_TGAKTGACAGG_motif_instances_0_mm.rds") %>% 
  keepStandardChromosomes(species="Mus_musculus", pruning.mode = "coarse")
...
pbx_motif_instances<-pbx_motif_instances[order(pbx_motif_instances$pbx_score, decreasing=T)][1:5000] 
```
